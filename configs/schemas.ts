...
export const StudentPersonalDetailsSchema = zod
  ...
  .refine((fieldsData) => fieldsData.mobile_no.length === 10 && fieldsData.mobile_no.match(/^[0-9]+$/) !== null, {
...
export const StudentAcademicDetailsSchema = zod.object({
  branch: zod.string({ required_error: "Branch is required" }),
  joining_year: zod.string({ required_error: "Joining Year is required" }),
  passing_year: zod.string({ required_error: "Passing Year is required" }),
